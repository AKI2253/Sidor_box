...
工作区右侧的轨道图标默认贴右缘、垂直居中。DSH 更新后右侧可能多出「对话内容选择栏」
等界面元素，轨道会**自动避让**，不与它们重叠：

- **取样原理**：用浏览器命中测试（`document.elementsFromPoint`）读取**默认锚点**
  （`right:12px` / `top:50%` 时的中心）上真实压着的元素，**不猜测 DOM 结构**；
