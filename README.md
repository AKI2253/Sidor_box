...
![版本](https://img.shields.io/badge/版本-v1.0.3-4f86f7?style=flat-square)
...
当前版本：**v1.0.3**（功能①–⑥完整；右侧轨道位置固定，面板展开即隐藏）。
...
工作区右侧的轨道图标**位置恒定**：`right: 12px` / `top: 50%` 垂直居中，代码里不写入任何
位置变量，**永不移位**。DSH 新版右侧会增加「对话内容选择栏 / 侧边工作区」等面板，处理方式是
**让位改为让路**——面板展开压住轨道占位时整体隐藏，收起后原位复现：

- **判定原理**：在轨道**默认占位矩形内部**（3×3 采样点）做浏览器命中测试
  （`document.elementsFromPoint`），判断是否有「贴右缘（`right ≥ 视口宽−24px`）且非背景」的元素；
  自动排除插件自身节点，以及整块背景（铺满视口，或又宽又高的主体内容 / 中心列——轨道本就浮在其上）。
- **隐藏规则**：压住 → `html.sid-rail-hidden` → 图标整体淡出（0.18s）；不再压住 → 自动恢复。
  展开 / 收起动画期间各需**连续 2 拍确认**才切换，避免动画中间态导致闪烁。
- **新对话创建界面**：`ConversationRoot` 的 `data-phase="hero"` 时同样隐藏，
  `settling` / `active` 显示；判定采用取值白名单（输入框另有同名 `data-phase`，
  取值为 `plain`/`claimed`/`submitting`/`adjudicating`/`inert`，与三值不相交）。
- **无自激回路**：采样点只依赖视口与轨道自身尺寸，**与轨道位置无关**，
  因此不存在"移动 → 采样变化 → 再移动"的反馈；位置与显隐都不抖。
- **实现**：1.2 秒轮询 + `resize` / `scroll` 节流 + `MutationObserver`（`data-phase` / 子节点变化）即时响应。
