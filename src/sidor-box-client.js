return {
  ...
  apply(ctx) {
    ...
    //
    // 与 hero 页（新对话创建界面）的显隐联动遵循三条时序规则，避免切换时“先出现再滑走”：
    //   1) hero 期间【冻结】位置，不复位、不重算；
    //   2) 回到对话页时【先算位置、再取消隐藏】，同一帧内完成，不会以旧位置先画一帧；
    //   3) 发现失败加迟滞（连续 3 次才复位），阶段变化时连发重算，兼顾目标挂载延迟。
    const RAIL_ANCHOR_RIGHT = 12
    const RAIL_EDGE = 12
    const RAIL_ANCHOR_HALF_W = 11
    const RAIL_MISS_LIMIT = 3
    const RAIL_GRACE_MS = 420
    let sidRailTarget = { el: null, mode: '' }
    let sidRailMiss = 0
    let sidRailPhasePrev = ''
    let sidRailGraceUntil = 0
    function sidRailEl() {
      const w = typeof window !== 'undefined' ? window : null
      if (!w || !w.document) return null
    ...
    // 纯粹计算位置（不涉及显隐）。返回是否已确定位置。
    function sidRailCompute() {
      const w = typeof window !== 'undefined' ? window : null
      if (!w || !w.document) return false
      const root = w.document.documentElement
      const vw = w.innerWidth
      ...
      if (!railEl) { sidRailTarget = { el: null, mode: '' }; sidRailReset(); return true }
      let railH = 0
      ...
      if (!r) {
        ...
        if (!found) {
          // 迟滞：遮挡物可能只是瞬时未挂载（页面切换/重排），保留现有位置，
          // 连续多次找不到才复位，避免在 hero↔对话 切换时来回跳动。
          sidRailMiss += 1
          if (sidRailMiss >= RAIL_MISS_LIMIT) {
            sidRailTarget = { el: null, mode: '' }
            sidRailReset()
          }
          return false
        }
    ...
    function sidRailApply() {
      ...
      if (phase !== sidRailPhasePrev) {
        sidRailPhasePrev = phase
        sidRailGraceUntil = hero ? 0 : (Date.now() + RAIL_GRACE_MS)
      }
      if (hero) {
        // 冻结：保留上次避让位，不重算也不复位 —— 回到对话时无需重新寻找就不会移动
        sidRailSetHidden(true)
        return
      }
      const determined = sidRailCompute()
      if (!determined && Date.now() < sidRailGraceUntil) {
        // 目标尚未挂载：先别显示，避免先以默认位画一帧再滑过去
        sidRailSetHidden(true)
        return
      }
    ...
    let sidRailBurstTimers = []
    function sidRailBurst() {
      const now = Date.now()
      if (now - sidRailBurstAt < 200) return
      sidRailBurstAt = now
      for (const d of [60, 160, 320, 620]) {
        sidRailBurstTimers.push(ctx.timeout(() => sidRailApply(), d))
      }
    }
    ...
    ctx.effect(() => {
      ...
      // 阶段属性变化（hero↔对话）立即重算 + 连发重算，覆盖目标挂载延迟
      let mo = null
      const MO = w && w.MutationObserver ? w.MutationObserver : null
      try {
        if (w && w.document && w.document.body && MO) {
          mo = new MO(() => { sidRailApply(); sidRailBurst() })
          mo.observe(w.document.body, { attributes: true, subtree: true, attributeFilter: ['data-phase'] })
        }
    ...
    function ToolboxSettingsPage() {
      ...
      return React.createElement('div', { className: 'sid-toolbox-page' },
        ...
        React.createElement('p', { className: 'sid-toolbox-page-desc' },
          'SIDOR 工具箱：常用工具与快捷功能的集合页。开启的功能会在工作区右侧轨道显示对应图标（仅对话内容中显示，新对话创建界面自动隐藏）。'),
...
.sid-rail {
  ...
  gap: 8px;
  pointer-events: none;
  transition: right 0.18s ease, top 0.18s ease, opacity 0.18s ease;
}
/* 新对话创建界面（hero）：会话根 data-phase="hero" 时隐藏轨道（对话内容中才显示） */
